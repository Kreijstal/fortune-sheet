...
export class Canvas {
    ...
    drawRowHeader(scrollHeight, drawHeight, offsetTop) {
        ...
        for (let r = dataset_row_st; r <= dataset_row_ed; r += 1) {
            ...
            renderCtx.closePath();
            // 行标题栏横线,horizen
            if (this.sheetCtx.config.rowhidden &&
            ...
            else if (this.sheetCtx.config.rowhidden == null ||
                this.sheetCtx.config.rowhidden[r] == null) {
                renderCtx.beginPath();
    ...
    drawColumnHeader(scrollWidth, drawWidth, offsetLeft) {
        ...
        for (let c = dataset_col_st; c <= dataset_col_ed; c += 1) {
            ...
            // 列标题栏竖线 vertical
            if (this.sheetCtx.config.colhidden &&
            ...
            else if (this.sheetCtx.config.colhidden == null ||
                this.sheetCtx.config.colhidden[c] == null) {
                renderCtx.beginPath();
    ...
    drawMain({ scrollWidth, scrollHeight, drawWidth, drawHeight, offsetLeft, offsetTop, columnOffsetCell, rowOffsetCell, clear, }) {
        ...
        colEnd = _.sortedIndex(this.sheetCtx.visibledatacolumn, scrollWidth + drawWidth);
        if (colStart === -1) {
            colStart = 0;
        }
    ...
    nullCellRender(r, c, startY, startX, endY, endX, renderCtx, afCompute, cfCompute, offsetLeft, offsetTop, dynamicArrayCompute, cellOverflowMap, colStart, colEnd, scrollHeight, scrollWidth, bodrder05, isMerge = false) {
        ...
        if (!cellOverflow_colInObj.colIn || cellOverflow_colInObj.colLast) {
            // 右边框
            // 无论是否有背景色，都默认绘制右边框
            if (!this.sheetCtx.luckysheetcurrentisPivotTable &&
