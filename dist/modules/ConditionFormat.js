            ...
            if (type === "dataBar") {
                ...
                if (!_.isNil(max) && !_.isNil(min)) {
                    if (min < 0) {
                        ...
                        const minusLen = Math.round((Math.abs(min) / (max - min)) * 10) / 10; // 负数所占比
                        for (let s = 0; s < cellrange.length; s += 1) {
                            for (let r = cellrange[s].row[0]; r <= cellrange[s].row[1]; r += 1) {
                                for (let c = cellrange[s].column[0]; c <= cellrange[s].column[1]; c += 1) {
                                    if (_.isNil(d[r]) || _.isNil(d[r][c])) {
                                    ...
                                    if (!_.isNil(cell) &&
                                        ...
                                        if (parseInt(`${cell.v}`, 10) < 0) {
                                            // 负数
                                            const valueLen = Math.round((Math.abs(parseInt(`${cell.v}`, 10)) /
                                                Math.abs(min)) *
                                                100) / 100;
                                        ...
                                        if (parseInt(`${cell.v}`, 10) > 0) {
                                            // 正数
                                            const valueLen = Math.round((parseInt(`${cell.v}`, 10) / max) * 100) /
                                                100;
                                            if (`${r}_${c}` in computeMap) {
                                                computeMap[`${r}_${c}`].dataBar = {
                                                    valueType: "plus",
                            ...
                            for (let r = cellrange[s].row[0]; r <= cellrange[s].row[1]; r += 1) {
                                for (let c = cellrange[s].column[0]; c <= cellrange[s].column[1]; c += 1) {
                                    ...
                                    if (!_.isNil(cell) &&
                                        ...
                                        if (parseInt(`${cell.v}`, 10) === min) {
                                        ...
                                        else if (parseInt(`${cell.v}`, 10) > min &&
                                            parseInt(`${cell.v}`, 10) < avg) {
                                            if (`${r}_${c}` in computeMap) {
                                                computeMap[`${r}_${c}`].cellColor = getColorGradation(format.cellColor, format.textColor, min, avg, parseInt(`${cell.v}`, 10));
                                            }
                                            else {
                                                computeMap[`${r}_${c}`] = {
                                                    cellColor: getColorGradation(format[2], format[1], min, avg, parseInt(`${cell.v}`, 10)),
                                                };
                                            }
                                        }
                                            ...
                                            if (`${r}_${c}` in computeMap) {
                                                computeMap[`${r}_${c}`].cellColor = getColorGradation(format[1], format[0], avg, max, parseInt(`${cell.v}`, 10));
                                            }
                                            else {
                                                computeMap[`${r}_${c}`] = {
                                                    cellColor: getColorGradation(format[1], format[0], avg, max, parseInt(`${cell.v}`, 10)),
                                                };
                                            }
                                        }
                                ...
                                for (let c = cellrange[s].column[0]; c <= cellrange[s].column[1]; c += 1) {
                                    ...
                                    if (!_.isNil(cell) &&
                                        ...
                                        if (parseInt(`${cell.v}`, 10) === min) {
                                            ...
                                            parseInt(`${cell.v}`, 10) < max) {
                                            if (`${r}_${c}` in computeMap) {
                                                computeMap[`${r}_${c}`].cellColor = getColorGradation(format[1], format[0], min, max, parseInt(`${cell.v}`, 10));
                                            }
                                            else {
                                                computeMap[`${r}_${c}`] = {
                                                    cellColor: getColorGradation(format[1], format[0], min, max, parseInt(`${cell.v}`, 10)),
                                                };
                                            }
                                        }
                                        else if (parseInt(`${cell.v}`, 10) === max) {
