...
export function clearFilter(ctx) {
    ...
    if (!allowEdit)
        return;
    const sheetIndex = getSheetIndex(ctx, ctx.currentSheetId);
    const hiddenRows = _.reduce(ctx.filter, (pre, curr) => _.assign(pre, curr?.rowhidden || {}), {});
...
export function createFilter(ctx) {
    ...
    ctx.luckysheet_filter_save = _.assign({}, filterSave?.[0] || ctx.luckysheet_select_save?.[0]);
...
function getFilterHiddenRows(ctx, col, startCol) {
    const otherHiddenRows = _.reduce(ctx.filter, (pre, curr) => _.assign(pre, (curr?.cindex !== col && curr?.rowhidden) || {}), {});
...
export function getFilterColumnColors(ctx, col, startRow, endRow) {
    // 遍历筛选列颜色
    const bgMap = new Map(); // 单元格颜色
    const fcMap = new Map(); // 字体颜色
