...
export function pasteHandlerOfPaintModel(ctx, copyRange) {
    ...
    const copyData = _.cloneDeep(getdatabyselection(ctx, { row: [c_r1, c_r2], column: [c_c1, c_c2] }, copySheetIndex));
    // 应用范围
                ...
                for (let c = mtc; c < maxcellCahe; c += 1) {
                    ...
                    if (value != null) {
                        ...
                        if (x[c].ct && x[c].ct.t === "inlineStr") {
                            x[c].ct.s.forEach((item) => _.assign(item, value));
                        }
                        if (copyHasMC && x[c].mc) {
                            if (x[c].mc.rs != null) {
                                x[c].mc.r = h;
    ...
    if (!copyIndex)
        return;
    const ruleArr = _.cloneDeep(ctx.luckysheetfile[copyIndex].luckysheet_conditionformat_save);
    if (!_.isNil(ruleArr) && ruleArr.length > 0) {
        ...
        for (let i = 0; i < ruleArr.length; i += 1) {
            ...
            for (let j = 0; j < cdformat_cellrange.length; j += 1) {
                const range = CFSplitRange(cdformat_cellrange[j], { row: [c_r1, c_r2], column: [c_c1, c_c2] }, { row: [minh, maxh], column: [minc, maxc] }, "operatePart");
                if (range.length > 0) {
...
export function moveHighlightRange(ctx, postion, index, type) {
    ...
    if (type === "rangeOfSelect") {
        ...
        const changeparam = mergeMoveMain(ctx, columnseleted, rowseleted, last, row_pre, row - row_pre - 1, col_pre, col - col_pre - 1);
...
export function calcSelectionInfo(ctx, lang) {
    ...
    const formatString = lang && !["zh", "zh_tw"].includes(lang) ? "0.00" : "w0.00";
