    ...
    if (tr === "3") {
        ...
        if (isInline) {
            ...
            for (let i = 0; i < inlineStringArr.length; i += 1) {
                ...
                const measureText = getMeasureText(value1, renderCtx, sheetCtx, shareCell.fontset);
                const textW = measureText.width + space_width;
                const textH = measureText.actualBoundingBoxAscent +
            ...
            const textHeight = measureText.actualBoundingBoxDescent +
        ...
        if (tb === "2" || isInline) {
            ...
            if (isInline) {
                while (i <= inlineStringArr.length) {
                    ...
                    if (shareCells[shareCells.length - 1].wrap === true) {
                        ...
                        if (shareCells.length === 1) {
                            ...
                            const measureText = getMeasureText("M", renderCtx, sheetCtx, sc.fontset);
                    ...
                    const width = textWidth * Math.cos((rt * Math.PI) / 180) +
                        textHeight * Math.sin((rt * Math.PI) / 180); // consider text box wdith and line height
                    const height = textWidth * Math.sin((rt * Math.PI) / 180) +
                ...
                while (i <= value.length) {
                    ...
                    const textHeight = measureText.actualBoundingBoxAscent +
                        measureText.actualBoundingBoxDescent;
                    const width = textWidth * Math.cos((rt * Math.PI) / 180) +
                        textHeight * Math.sin((rt * Math.PI) / 180); // consider text box wdith and line height
                    const height = textWidth * Math.sin((rt * Math.PI) / 180) +
                ...
                for (let j = 0; j < splitLen; j += 1) {
                    ...
                    for (let c = splitLists.length - 1; c >= 0; c -= 1) {
                        ...
                        if (rt !== 0) {
                            ...
                            const x = cumWordHeight / Math.tan(rtPI) -
                                cumColumnWidth +
                                textW_all_inner;
                            if (horizonAlign === "0") {
                                // center
